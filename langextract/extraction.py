...
def extract(
    text_or_documents: typing.Any, # 要进行抽取的文本或文档
    prompt_description: str | None = None,  # 抽取指令的描述
    examples: typing.Sequence[typing.Any] | None = None,  # 抽取指令的示例
    model_id: str = "gemini-2.5-flash",
    api_key: str | None = None,
    language_model_type: typing.Type[typing.Any] | None = None,
    format_type: typing.Any = None,  # 抽取指令的输出格式
  ...
  Args:
      ...
      runtime_observer_config: Optional runtime observation config used to
        write runtime observation JSON artifacts.
  ...
  """
  # B. Task definition guard: require few-shot examples for reliable extraction.
  if not examples:
    raise ValueError(
  ...
  # C. Pre-flight prompt validation for example/label alignment.
  ...
  if (
    ...
    # A. Input normalization: if input is a URL, fetch text first.
    text_or_documents = io.download_text_from_url(text_or_documents)

  # B. Build the prompt template from description + examples.
  ...
  if model:
    # F/G. Use injected model directly (Annotator handles prompt assembly + infer).
    ...
    # G. Build provider/model from explicit config (optionally with schema constraints).
    if use_schema_constraints:
      warnings.warn(
    ...
    # G. Default model path: create provider/model from model_id + kwargs.
  ...
  # H. Configure output format handling and validate schema compatibility.
  ...
  try:
    # I/J. Resolver parses model output into Extraction and applies alignment settings.
  ...
  if isinstance(text_or_documents, str):
    # A->N single-text pipeline handled inside Annotator:
    # D tokenization + E chunking + F prompt build + G batched infer
    # + H/I/J parse/resolve/alignment + K pass merge/dedup + L output document.
    ...
    # A->N multi-document pipeline: same stages as above, iterated per document.

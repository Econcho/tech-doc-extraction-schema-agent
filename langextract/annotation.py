...
from langextract.runtime_observer import activate_observer_session
from langextract.runtime_observer import RuntimeObservationManager
...
def _document_chunk_iterator(
  ...
  for document in documents:
    ...
    '''
    创建ChunkIterator实例。分chunk策略是懒启动，在迭代ChunkIterator时才会生成chunk。
    '''
    visited_ids.add(document_id)
    policy = chunk_policy or SentenceChunkPolicy()
    observer_session = None
    if runtime_observer and runtime_observer.enabled:
      observer_session = runtime_observer.get_session(document_id)
    with activate_observer_session(observer_session):
      yield from policy.iter_chunks(
          document=document,
...
class Annotator:
  ...
  def annotate_documents(
      ...
      tokenizer: tokenizer_lib.Tokenizer | None = None,
      chunk_policy: ChunkPolicy | None = None,
      runtime_observer_config: RuntimeObserverConfig | dict | None = None,
      **kwargs,
    ...
    Args:
      ...
      runtime_observer_config: Optional runtime observation config.
  ...
  def _annotate_documents_single_pass(
    ...
    next_emit_idx = 0
    observer_config = RuntimeObserverConfig.from_value(
        runtime_observer_config
    )
    observer_manager = RuntimeObservationManager(
        config=observer_config,
        model_name=getattr(
            self._language_model,
            "model_id",
            self._language_model.__class__.__name__,
        ),
        max_char_buffer=max_char_buffer,
        chunk_policy=chunk_policy,
        pass_index=observer_pass_index,
    )
    ...
    def _capture_docs(src: Iterable[data.Document]) -> Iterator[data.Document]:
      ...
      for document in src:
        ...
        observer_manager.start_document(document)
        yield document
    ...
    def _emit_docs_iter(
      ...
      while next_emit_idx < limit:
        ...
        observer_manager.close_document(document_id, status="success")
    ...
    chunk_iter = _document_chunk_iterator(
        ...
        tokenizer=tokenizer,
        chunk_policy=chunk_policy,
        runtime_observer=observer_manager,
    )
    '''
    将chunk分batch
    '''
    ...
    try:
      ...
      for batch in batch_iter:
        ...
        if not batch:
          continue
        '''
        batch中每个chunk构建一个prompt，一个batch生成一个prompt列表
        '''
        prompts: list[str] = []
        observer_prompt_contexts: list[dict[str, object] | None] = []
        for text_chunk in batch:
          prompt_parts = prompt_builder.build_prompt_parts(
              text_chunk.chunk_text,
              text_chunk.document_id,
              text_chunk.additional_context,
          )
          prompts.append(prompt_parts.prompt)

          observer_session = observer_manager.get_session(
              text_chunk.document_id
          )
          if observer_session is None:
            observer_prompt_contexts.append(None)
            continue

          chunk_id = observer_session.ensure_chunk_id(text_chunk)
          observer_session.record_prompt(
              chunk_id=chunk_id,
              chunk_text=prompt_parts.chunk_text,
              additional_context=prompt_parts.additional_context,
          )
          observer_prompt_contexts.append({
              "recorder": observer_session,
              "chunk_id": chunk_id,
          })
        ...
        if (
            self._language_model.__class__.__module__
            == "langextract.providers.openai"
        ):
          infer_kwargs["_runtime_observer_prompt_contexts"] = (
              observer_prompt_contexts
          )
        outputs = self._language_model.infer(
            batch_prompts=prompts, **infer_kwargs
        )
        if not isinstance(outputs, list):
          outputs = list(outputs)

        for text_chunk, scored_outputs, observer_context in zip(
            batch, outputs, observer_prompt_contexts
        ):
          ...
          observer_session = None
          if observer_context:
            observer_session = observer_context["recorder"]
            chunk_id = observer_context["chunk_id"]
            llm_call_id = observer_context.get("llm_call_id")
          '''
          只取top-1进行resolve
          '''
          try:
            ...
            if observer_session and chunk_id:
              observer_session.record_parse(
                  chunk_id=chunk_id,
                  llm_call_id=llm_call_id,
                  parse_status="success",
                  parsed_extraction_count=len(resolved_extractions),
              )
          ...
          for extraction_index, extraction in enumerate(aligned_extractions):
            per_doc[text_chunk.document_id].append(extraction)
            if observer_session and chunk_id:
              observer_session.record_extraction(
                  chunk_id=chunk_id,
                  llm_call_id=llm_call_id,
                  extraction_index_in_chunk=extraction_index,
                  extraction=extraction,
              )
          args = (
              "annotation.py-Annotator._annotate_documents_single_pass-"
              "per_doc[text_chunk.document_id]"
          )
          args1 = per_doc[text_chunk.document_id]
      ...
      observer_manager.close_all_pending(
          status="failure",
          error_message=str(e),
      )
      raise

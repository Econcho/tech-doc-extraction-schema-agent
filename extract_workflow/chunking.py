...
class TextChunk:
  ...
  def additional_context(self) -> str | None:
    ...
    if self.document is not None and self.document.additional_context:
      context_parts.append(self.document.additional_context)

    heading_context = self._build_heading_context()
    if heading_context:
      context_parts.append(heading_context)

    return "\n\n".join(context_parts) if context_parts else None

  def _build_heading_context(self) -> str | None:
    """Builds heading context text from any attached context refs."""
    ...
    for context_ref in self.context_refs:
      if context_ref.ref_type != "heading_path" or not context_ref.context_text:
        continue
      normalized = context_ref.context_text.strip()
...
class ChunkIterator:
  ...
  * "And so are you." len=15
  ...
  def _build_text_chunk(
      self, token_span: tokenizer_lib.TokenInterval
  ) -> TextChunk:
    char_span = get_char_interval(self.tokenized_text, token_span)
    text_chunk = TextChunk(token_interval=token_span, document=self.document)
    return text_chunk
  ...
  def _tokens_exceed_buffer(
    ...
    判断某一段 token 对应的文本长度是否超过 max_char_buffer
  ...
  def __next__(self) -> TextChunk:
    """
    真正获取下一个chunk的核心方法
    chunk策略：以“句子边界优先、字符上限约束、必要时在换行处截断”的贪心切块策略。
    1. 如果当前token超过 max_char_buffer，直接返回当前token作为chunk
    2. 如果当前token不超过 max_char_buffer，从当前句子的起始 token 开始，尽量往后加 token，直到再加就会超出 max_char_buffer。
       如果句子本身太长，它会优先考虑在换行边界处断开，利用 token 上的 first_token_after_newline 标记，使切分更贴近原文本结构。
    3. 如果当前句子还没把 buffer 填满，它会继续尝试把后续完整句子拼进来，直到即将超长为止
    """
    ...
    if self._tokens_exceed_buffer(curr_chunk):
      ...
      return self._build_text_chunk(curr_chunk)

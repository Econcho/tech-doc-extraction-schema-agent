...
class OpenAILanguageModel(base_model.BaseLanguageModel):
  ...
  def _process_single_prompt(
    ...
    try:
      ...
      _hook_raw_output(
          'openai.py-OpenAILanguageModel._process_single_prompt-output_text',
          output_text,
          note=f'model_id={self.model_id}',
          max_chars=8000,
      )
      if observer_context and observer_context.get('recorder'):
        llm_call_id = observer_context['recorder'].record_llm_call(
        ...
        observer_context['llm_call_id'] = llm_call_id
  ...
  def _extract_usage(self, response: Any) -> dict[str, int] | None:
    """Extract token usage from one OpenAI response if available."""
  ...
  def infer(
    ...
    observer_contexts = merged_kwargs.pop(
        '_runtime_observer_prompt_contexts', None
    )
    ...
    if len(batch_prompts) > 1 and self.max_workers > 1:
      with concurrent.futures.ThreadPoolExecutor(
        ...
        future_to_index = {
            executor.submit(
                ...
                (
                    observer_contexts[i]
                    if observer_contexts is not None
                    else None
                ),
            ): i
      ...
      for i, prompt in enumerate(batch_prompts):
        ...
        if observer_contexts is not None:
          observer_context = observer_contexts[i]
